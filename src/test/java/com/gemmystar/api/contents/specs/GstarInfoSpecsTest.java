...
public class GstarInfoSpecsTest {
	...
	@Autowired
	GstarInfoRepository infoRepo;

	@Test
	public void testTopGstarInfos() {
		Specifications<GstarInfo> infoSpec = Specifications.where(GstarInfoSpecs.topGstarInfos(1L));
		
		try{
			List<GstarInfo> list = infoRepo.findAll(infoSpec);
		} catch(Exception e) {
			fail(e.toString());
			e.printStackTrace();
		}
