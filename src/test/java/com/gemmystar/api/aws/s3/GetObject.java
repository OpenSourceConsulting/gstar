...
*/
        ...
        String key_name = "video/C17_zvieDaedgQQ.mp4";
        
        System.out.format("Downloading %s from S3 bucket %s...\n", key_name, bucket_name);
        final AmazonS3 s3 = new AmazonS3Client();
        File destinationFile = new File("G:/down/" + key_name);
        final GetObjectRequest getObjectRequest = new GetObjectRequest(bucket_name, key_name);
        try {
        	/*
            S3Object o = s3.getObject(bucket_name, key_name);
            S3ObjectInputStream s3is = o.getObjectContent();
            FileOutputStream fos = new FileOutputStream(new File("G:/down/" + key_name));
            byte[] read_buf = new byte[1024*10];
            int read_len = 0;
